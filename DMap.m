function [LocHist LocHistCount] = DMap(tracks, subpixel, holdFigure)

if holdFigure
    figureHandle = params.figureHandle;    
end

xedges = min(tracks(:,1)):subpixel:max(tracks(:,1));
yedges = min(tracks(:,2)):subpixel:max(tracks(:,2));

[LocHist, LocHistCount] = HIST2(tracks(:,1),tracks(:,2),xedges,yedges);
...
%%
function [MN, MNcount, xedges, yedges] = HIST2(x, y, xedges, yedges)
...
[MN, MNcount] = histc(xyBin,xyBinEdges);

function [pos1,pos2] = gaussStormSingleParticle(imagename, thresh, windowSize, frame, pos, writeBin)
%function daoSTORMsimple(imagename, windowSize)

savename    = [imagename(1:end-5),'_thresh',num2str(thresh),'.gaussstorm.mod.pos.out'];
...
if isempty(pos)==0
    for j = 1:size(pos,1)%for each particle
        ...
        if ~all(a == 0 )%if the fit has worked
            ...
            if writeBin%if the fit has worked
